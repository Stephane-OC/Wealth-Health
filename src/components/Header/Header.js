...
import { faMoon, faSun, faBars, faTimes, faUserGroup } from "@fortawesome/free-solid-svg-icons";
...
const Header = () => {
    ...
    return (
      <header className="header">
        ...
        <nav className={`navigation ${isMobile && isMobileNavVisible ? "show" : ""}`}>
          ...
          {isMobile ? (
            <Link to="/employee-list" className="nav-link" aria-label="Employee list">
              <FontAwesomeIcon icon={faUserGroup} />
              <span className="title-2">Employee</span>
            </Link>
          ) : (
            <Link className="title-2" to="/employee-list">View Employees</Link>
          )}
          <button
